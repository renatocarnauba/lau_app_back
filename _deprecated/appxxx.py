...
url_do_banco = "postgresql+asyncpg://lau:J$JW^OJaZ7k6@localhost:5432/lau"
...
async def buscar_pessoa(nome="Felipe"):
    async with session() as s:
        ...
        query = await s.execute(select(Pessoa).where(Pessoa.nome == nome))
...
async def deletar_pessoa(nome):
    async with session() as s:
        await s.execute(delete(Pessoa).where(Pessoa.nome == nome))
        await s.commit()
...
async def buscar_post_por_autor(nome):
    async with session() as s:
        query = await s.execute(
            select(Pessoa, Post).join(Post.autor).where(Pessoa.nome == nome)
        )
        result = query.first()
        # result = query.scalar()
        breakpoint()
